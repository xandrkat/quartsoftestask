...
class SiteController extends Controller
{
     ...
     * @return mixed
     */
    public function actionCadastr()
    {
        $searchModel = new CadastrSearch();
        $dataProvider = $searchModel->search($this->request->queryParams);

        return $this->render('cadastr', [
    ...
    public function actionUpdate()
    {
        ...
        foreach ($models as $model) {
            $response = \yii\helpers\Json::decode($client->get('/open/cadastral/find-center-by-cadastral-number', [
                'clientId'        => Yii::$app->params['clientId'],
                'cadastralNumber' => $model->cn
            ])->send()->content);
            if ($response['status']) {
